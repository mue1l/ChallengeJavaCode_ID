...
public class Main {
    public static void main(String[] args) {
        ...
        List<VehiclePurchase> purchases = List.of(vhc1, vhc2, vhc3, vhc4, vhc5, vhc6, vhc7, vhc8, vhc9, vhc10);
        ...
        for (VehiclePurchase v : purchases) {
            System.out.println(v);

            totalTax += v.getTax();

            switch (v.getVehicletype()) {
                case PrivateJet -> incomePlane += v.getTotal();
                case Boat -> incomeBoat += v.getTotal();
                default -> incomeCar += v.getTotal();
            }
        }

        System.out.printf("TotalIncomeCar : %d%n", incomeCar);
        System.out.printf("TotalIncomePlane : %d%n", incomePlane);
        System.out.printf("TotalIncomeBoat : %d%n", incomeBoat);
        System.out.printf("TotalTax : %d%n", totalTax);
        System.out.printf("TotalCar : %d%n", VehiclePurchase.totalCar);
        System.out.printf("TotalPlane : %d%n", VehiclePurchase.totalPlane);
        System.out.printf("TotalBoat : %d%n", VehiclePurchase.totalBoat);
    }
}